...
- **Ethics, bias, fairness analysis.** Dedicated notebook [`05_ethics_and_bias.ipynb`](notebooks/05_ethics_and_bias.ipynb) covering: geographic bias (USA-dominant), temporal bias (2017-2019 only), brand imbalance (Off-White/Yeezy oversampled), ML residuals by brand, **quantitative per-brand CV fairness on a 250-image held-out tail** (adidas-yeezy F1 ≈ 0.55 vs new_balance / reebok F1 ≈ 0.10 – confirms the rare-brand penalty predicted by the dataset composition), socioeconomic considerations of the resell market, explicit limitations (no fake detection, no real-time pricing, no condition assessment), and in-app disclaimer policy. Numbers are persisted to [`data/processed/cv_per_brand_f1.csv`](data/processed/cv_per_brand_f1.csv); chart at [`assets/screenshots/cv_per_brand_f1.png`](assets/screenshots/cv_per_brand_f1.png).
