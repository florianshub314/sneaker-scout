...
CV_CELLS = [
    ...
    md("## Model 1 – ViT Fine-Tuning\n\n"
       ...
       "our class count, then fine-tune end-to-end. If a previously trained model exists at "
       "`models/cv_model/`, it is loaded directly to skip the (slow) training step."),
    ...
    code("def compute_metrics(eval_pred):\n"
         ...
         "VIT_ALREADY_TRAINED = (config.CV_MODEL_DIR / 'model.safetensors').exists()\n\n"
         "if VIT_ALREADY_TRAINED:\n"
         "    print(f'Loading pretrained ViT from {config.CV_MODEL_DIR}')\n"
         "    vit_model = ViTForImageClassification.from_pretrained(str(config.CV_MODEL_DIR))\n"
         "else:\n"
         "    print('Initialising ViT from base checkpoint')\n"
    ...
    code("training_args = TrainingArguments(\n"
         ...
         ")\n\n"
         "if not VIT_ALREADY_TRAINED:\n"
         "    train_result = trainer.train()\n"
         "    print(train_result)\n"
         "else:\n"
         "    print('Skipping training – reusing saved fine-tuned model.')"),

    md("### Save the fine-tuned ViT"),
    code("if not VIT_ALREADY_TRAINED:\n"
         ...
         "else:\n"
         "    print(f'Model already saved at {config.CV_MODEL_DIR}')"),
    ...
    md("## Model 2 – ResNet50 Baseline (Frozen Backbone)\n\n"
       "Compare against a torchvision ResNet50 used as a feature extractor: backbone is "
       "frozen, only the classifier head is trained. This is a standard transfer-learning "
       "baseline that runs in minutes and isolates the contribution of fine-tuning the "
       "backbone (which is what ViT receives). Trained on CPU because torchvision ResNet "
       "has known deadlock issues on Apple Silicon MPS during long runs."),

    code("# Build a frozen-backbone ResNet50: only the classifier head is learnable\n"
         "resnet_device = torch.device('cpu')  # MPS deadlocks on long torchvision runs\n"
         "backbone = models.resnet50(weights=models.ResNet50_Weights.DEFAULT)\n"
         "in_features = backbone.fc.in_features\n"
         "backbone.fc = torch.nn.Identity()  # remove final FC; we extract features\n"
         "for p in backbone.parameters():\n"
         "    p.requires_grad = False\n"
         "backbone = backbone.to(resnet_device).eval()\n\n"
         "classifier = torch.nn.Linear(in_features, len(class_names)).to(resnet_device)\n"
         ...
         "        x = batch['pixel_values'].to(resnet_device)\n"
         "        y = batch['labels'].to(resnet_device)\n"
         "        with torch.no_grad():\n"
         "            features = backbone(x)\n"
         "        optimizer.zero_grad()\n"
    ...
    code("# ResNet test predictions\n"
         ...
         "        x = batch['pixel_values'].to(resnet_device)\n"
         "        feats = backbone(x)\n"
         "        pred = classifier(feats).argmax(-1).cpu().numpy()\n"
         ...
         "print(f'ResNet50 (frozen) test: acc={resnet_acc:.4f} f1={resnet_f1:.4f}')"),
